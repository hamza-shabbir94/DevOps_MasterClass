# https://dev.to/korakrit/installing-kubernetes-and-worker-node-setup-on-ubuntu-2404-2oag
...
# it after running this on the worker you get error "E0415 10:50:46.759721   19680 memcache.go:265] "Unhandled Error" err="couldn't get current server API group list: Get \"http://localhost:8080/api?timeout=32s\": dial tcp [::1]:8080: connect: connection refused"
...
# 2. Securely copy the admin config from your Master node to this Worker node
# (Replace 'master-ip' with the actual IP of your master node)
scp root@159.203.186.137:/etc/kubernetes/admin.conf $HOME/.kube/config

# 3. Set the correct permissions so your user can read it
sudo chown $(id -u):$(id -g) $HOME/.kube/config